...
A personal [Homebrew](https://brew.sh) tap — prebuilt CLI tools for macOS and Linux.
...
<!-- BEGIN TOOLS (auto-generated from Formula/*.rb — do not edit by hand) -->
...
- **The tool list above is generated too.** `scripts/gen-readme.py` reads every
  `Formula/*.rb` and rewrites the table between the `BEGIN TOOLS` / `END TOOLS`
  markers. A GitHub Actions workflow runs it on any push that touches
  `Formula/`, so the catalog can't drift — nothing here is a hand-kept list.
