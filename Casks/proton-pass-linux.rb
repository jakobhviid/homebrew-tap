cask "proton-pass-linux" do
  version "1.40.0"
  sha256 x86_64_linux: "dfa698d4a724c2477248286df925148ce4c0b8f783073ee51d17dea4cab7dd4c"
  ...
  # derivable from `version`. scripts/bump-proton.py rebuilds this URL from the
  # feed and fails loudly if the shape it produces stops matching, rather than
  ...
  # ../lib/proton-pass/, so it keeps resolving inside the Caskroom once staged and
  # Homebrew's bin symlink resolves through it to the real Electron binary.
  # Pointing at "Proton Pass" directly would work too but would hardcode a name
  # that carries upstream's "Beta" suffix on some apps and not others.
  binary "usr/bin/proton-pass"
  artifact "usr/share/applications/proton-pass.desktop",
           target: "#{Dir.home}/.local/share/applications/proton-pass.desktop"
  # Why installed as .svg: as with Mail, upstream ships /usr/share/pixmaps/
  # proton-pass.png containing SVG data. Renamed so the icon theme indexes it.
  artifact "usr/share/pixmaps/proton-pass.png",
           target: "#{Dir.home}/.local/share/icons/hicolor/scalable/apps/proton-pass.svg"
  ...
  preflight_steps do
    ...
    # Why: upstream ships `Exec=proton-pass %U`, a bare command resolved through
    ...
    inreplace "usr/share/applications/proton-pass.desktop",
              /^Exec=.*$/,
              "Exec={{HOMEBREW_PREFIX}}/bin/proton-pass %U"
  end
  ...
  # Why this path: Electron derives userData from productName, following the same
  # convention Mail is observed to use ("Proton Mail", space, no "Beta").
  zap trash: [
    "~/.cache/Proton Pass",
    "~/.config/Proton Pass",
  ]
  ...
  caveats <<~EOS
    Homebrew owns this install's version. Upgrade with:
      brew upgrade --cask proton-pass-linux

    Proton Pass is Electron, so it links against a desktop runtime (GTK 3, NSS,
