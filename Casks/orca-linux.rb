cask "orca-linux" do
  ...
  # after the next login — orca-ide registers x-scheme-handler/orca and
  # text/markdown, so mimeinfo.cache is what makes those associations real.
  # `must_succeed: false` keeps each step a no-op where the tool is not shipped.
  #
  ...
  # no `{{home}}` token (the runner's list is prefix/staged_path/appdir and
  # friends, plus `{{user}}`), `chdir` resolves only against the step's default
  # base, and interpolating #{Dir.home} is rejected by the style cop, which
  # allows only step DSL calls and literal arguments inside a steps block.
  # Hardcoding /home is safe here because the cask is Linux-only.
  postflight_steps do
